...
<div>
    <q-list v-show="!shrink" separator>
        <template v-for="menu in menuList" v-if="menu.leftMemu">
                <q-collapsible v-model="menu.open" group="somegroup" indent v-if="menu.children&&menu.children.length>0" :icon="menu.icon" :label="menu.title" :key="menu.name">
                    <template v-for="child in menu.children" v-if="child.leftMemu">
                        <q-collapsible v-if="child.children&&child.children.some(s=>{return s.leftMemu})" :key="child.name" :icon="child.icon" indent :label="child.title">
                            <template v-for="child1 in child.children" v-if="child1.leftMemu">
                                <q-item link :to="{ name: child1.name}" :key="child1.name">
                                    ...
                                    <q-item-main>
                                        <q-item-tile v-ripple label>{{child1.title}}</q-item-tile>
                                    </q-item-main>
                                </q-item>
</template>
                        </q-collapsible>
                        <q-item v-else link :to="{ name: child.name}" :key="child.name">
                            ...
                            <q-item-main>
                                <q-item-tile v-ripple label>{{child.title}}</q-item-tile>
                            </q-item-main>
                        </q-item>
                    </template>
                </q-collapsible>
                <q-collapsible indent v-else :icon="menu.icon" :label="menu.title" :key="menu.name">
                    <q-item link :to="{ name: menu.name}">
                        <q-item-side :icon="menu.icon" />
                        <q-item-main>
                            <q-item-tile v-ripple label>{{menu.title}}</q-item-tile>
                        </q-item-main>
                    </q-item>
                </q-collapsible>
            </template>
